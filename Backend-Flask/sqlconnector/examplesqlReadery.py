    ...
    try:
        with db.connect() as conn:
            ...
            charEntries = conn.execute(
                sqlalchemy.text("""SELECT `p`.`PlayerName`, `c`.`CharacterName`, `c`.`ClassName`
            ...
            roleEntries = conn.execute(
                sqlalchemy.text("""SELECT c.CharacterName, pr.PartyRoleName, c.ClassName, cr.RoleSkill FROM `character` as c
        ...
        for row in charEntries:
            ...
            for key, value in player_dict.items():
                ...
                if key == row[0] and (len(value) == 0):
                    ...
                    player_dict[row[0]] = {
                        row[1]: {"Class": row[2]}
                    }  # <--- where to add a characters information to the dictionary (Character : {Class: Priest, Key: 14, etc}) from sql query
                    # print(f"Added {row}")
                    # print(player_dict)
                    # print("")
                elif key == row[0]:
